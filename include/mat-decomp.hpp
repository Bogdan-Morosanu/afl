...
bool Mat<ElType, rows, cols>::inv(Mat &inverse) const {
...
template <typename ElType, int rows, int cols>
bool Mat<ElType, rows, cols>::lup_decomp(
    Mat &l, Mat &u, Mat &p) const { // notice templated member functions are
                                    // only instantiated on calling,
...
template <typename ElType, int rows, int cols>
inline bool Mat<ElType, rows, cols>::solve(const Mat<ElType, rows, 1> &rhs,
  ...
  default:
    assert(false && "undefined system of equation type "
                    "passed to Mat::solve_for, please review sources");
  }
}

template <typename ElType, int rows, int cols>
inline bool Mat<ElType, rows, cols>::solve_from_lup(
    const Mat<ElType, rows, 1> &rhs, const Mat &l, const Mat &u, const Mat &p,
...
template <typename ElType, int rows, int cols>
inline bool
Mat<ElType, rows, cols>::solve_general(const Mat<ElType, rows, 1> &rhs,
                                       Mat<ElType, rows, 1> &sol) const {
  Mat l, u, p;
...
template <typename ElType, int rows, int cols>
bool Mat<ElType, rows, cols>::back_subst(const Mat<ElType, rows, 1> &rhs,
                                         Mat<ElType, rows, 1> &sol) const {
  ...
  for (int r = rows - 1; r > -1; --r) {
    ...
    // TODO maybe guard with a flag for no check? (well, outside for loop
    // obviously)
...
template <typename ElType, int rows, int cols>
bool Mat<ElType, rows, cols>::fwd_subst(const Mat<ElType, rows, 1> &rhs,
                                        Mat<ElType, rows, 1> &sol) const {
  sol = rhs; // sol will also keep our temp, mutable rhs vector.
