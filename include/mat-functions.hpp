...
Mat<ElType, dim, dim> eye() { // dim * dim assignments + dim * dim comparisons
...
template <typename ElType, int dim>
Mat<ElType, dim, dim>
permutation(const std::array<int, dim>
                &p) { // dim * dim assignments + dim * dim comparisons
  // are probably more expensive than
  // dim * dim + dim assignments, so we don't bother
  // checking. init all to zero, make diag 1.
  Mat<ElType, dim, dim> retval(0);
...
inline SeedType reduce_with(std::function<SeedType(SeedType, ElType)> reducer,
                            const Mat<ElType, rows, cols> &mat, SeedType seed,
                            IterationDir dir) {
  for (int c = 0; c < cols; ++c) {
    for (int r = 0; r < rows; ++r) {
...
inline SeedType reduce_with(const Mat<ElType, rows, cols> &mat, SeedType seed) {
  for (int c = 0; c < cols; ++c) {
    for (int r = 0; r < rows; ++r) {
      seed = reducer(seed, mat(r, c));
    }
...
inline Mat<ElType, rows, cols> operator*(const Mat<ElType, rows, n> &lhs,
  ...
  intern::overload_resolver_gemm(
      ...
      CBLAS_TRANSPOSE::CblasNoTrans, rows, cols, n, 1.0, lhs.buf[0], rows,
      rhs.buf[0], n, 0.0, retval.buf[0], rows);
