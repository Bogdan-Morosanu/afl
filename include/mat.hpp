...
template <typename ElType, int cols> using ColVect = Mat<ElType, 1, cols>;

/// @brief factory function, returns Identity matrix
template <typename ElType, int dim> Mat<ElType, dim, dim> eye();
...
Mat<ElType, dim, dim> permutation(const std::array<int, dim> &p);
...
/// (0,0) will be (0,1) ).
template <typename SeedType, typename ElType, int rows, int cols>
inline SeedType reduce_with(std::function<SeedType(SeedType, ElType)> reducer,
...
inline SeedType reduce_with(const Mat<ElType, rows, cols> &mat,
                            SeedType seed = SeedType(0));
...
public:
  ...
  /// (1, 0) ).
  inline ElType operator()(int idx,
                           IterationDir dir = IterationDir::COLS) const;
  ...
  /// then if index is 1, function will try to index elem (0, 1), otherwise
  /// (1, 0) ).
  inline ElType &operator()(int idx, IterationDir dir = IterationDir::COLS);

  /// access constant marix.
  inline ElType operator()(int row, int col) const;

  /// access non-const matrix.
  inline ElType &operator()(int row, int col);
  ...
  inline friend Mat<FnElType, r, c> operator*(const Mat<FnElType, r, n> &lhs,
  ...
  inline std::pair<ElType, int> min(IterationDir d, int idx) const;
  ...
  inline std::pair<ElType, int> min_abs(IterationDir d, int idx) const;
  ...
  inline std::pair<ElType, int> max(IterationDir d, int idx) const;
  ...
  inline std::pair<ElType, int> max_abs(IterationDir d, int idx) const;

  /// @brief returns transposed matrix.
  inline Mat<ElType, cols, rows> transp() const;

  /// @brief returns matrix inverse.
  bool inv(Mat &inverse) const;

  /// @brief performs L-U-P decomposition of this.
  /// @return success status (fails if matrix is singular).
  bool lup_decomp(Mat &l, Mat &u, Mat &p) const;
  ...
  inline bool solve(const Mat<ElType, rows, 1> &rhs, Mat<ElType, rows, 1> &sol,
  ...
  /// @param sol solution, output variable
  /// @return success status.
  inline bool solve_from_lup(const Mat<ElType, rows, 1> &rhs, const Mat &l,
                             const Mat &u, const Mat &p,
                             Mat<ElType, rows, 1> &sol) const;

  /// @brief returns sum of squares of elements.
  inline ElType l2_norm() const;

  /// @brief return sum of elements.
  inline ElType l1_norm() const;

  /// @brief returns max element.
  inline ElType linf_norm() const;

private:
  /// solves general systems of equations
  inline bool solve_general(const Mat<ElType, rows, 1> &rhs,
                            Mat<ElType, rows, 1> &sol) const;
