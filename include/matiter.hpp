...
template <typename ElType> class Seq {
public:
  Seq(ElType *begin, ElType *end) : b(begin), e(end) {}
  ...
  bool operator==(const Seq &rhs) const { return b == rhs.b; }
...
template <typename ElType, IterationDir Dir = IterationDir::COLS>
class MatIter {
  /// @brief Mat is a friend class since only
  /// matrices can create matrix iterators.
  template <int rows, int cols> friend class Mat;
...
private:
  MatIter() = delete;
  MatIter(std::size_t row_size, std::size_t col_size, std::size_t row_start,
