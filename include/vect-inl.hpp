...
inline Vect<ElType, dim> Vect<ElType, dim>::normalized() const {
  return ElType(1) / this->l2_norm() * (*this);
}

template <typename ElType, int dim>
inline Vect<ElType, dim> Vect<ElType, dim>::projected(
    const Vect &rhs) const { // we can't just do rhs * rhs since that's not
...
template <typename ElType, int dim>
inline Vect<ElType, dim>
Vect<ElType, dim>::orthogonalized(const Vect &axis) const {
