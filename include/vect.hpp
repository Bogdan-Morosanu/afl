...
public:
  Vect(ElType val) : Mat<ElType, dim, 1>(val) {}

  Vect(const std::initializer_list<ElType> &ls) : Mat<ElType, dim, 1>(ls) {}

  /// @brief returns projection of *this on rhs.
  inline Vect projected(const Vect &rhs) const;
  ...
  inline Vect orthogonalized(const Vect &axis) const;
